...
public:
  ...
  /// Принимает обучающие данные (пары вход-выход), количество эпох, размер мини-батча, скорость обучения
  /// Для каждой эпохи
  ...
  /// веса и смещения. Для каждой пары в мини-батче вызывает backprop для получения градиентов,
  /// накапливает градиенты. Обновляет веса и смещения по формуле
  /// параметр = параметр - (скорость обучения/размер батча)*градиент
